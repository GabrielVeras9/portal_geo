...
@Injectable()
export class UrbiService {
  private readonly logger = new Logger(UrbiService.name);
  ...
  async fetchAndSaveData() {
    const gpsConfig = this.configService.get('gps.urbi');
    const url = `${gpsConfig.scheme}://${gpsConfig.host}${gpsConfig.path}`;
    const config = {
      method: 'get',
      url: url,
      headers: { 
